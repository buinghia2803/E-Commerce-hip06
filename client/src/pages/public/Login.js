...
const Login = () => {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    if (isRegister) {
      ...
      if (rs.success) {
        dispatch(register({ isLoggedIn: true, token: rs.accessToken, current: rs.mes }))
        navigate(`/${path.HOME}`)
      ...
      <div className='absolute top-0 bottom-0 left-0 right-1/2 flex items-center justify-center'>
        <div className='p-8 bg-white flex flex-col items-center rounded-md min-w-[500px]'>
          ...
          <div className='flex items-center justify-between w-full text-sm my-2'>
            {!isRegister && <span className='text-blue-500 hover:underline cursor-pointer'>Forgot acc</span>}
            {!isRegister && <span
